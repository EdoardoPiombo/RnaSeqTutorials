...
tutorial:
  id: "org.bnbio.tutorials.06_differential_transcript_usage"
  version: 0.1
output:
  learnr::tutorial:
    progressive: true
...
# to run locally replace www with tutorials/06_differential_transcript_usage/www
...
```{r prep, exercise=TRUE, exercise.eval=TRUE}
suppressPackageStartupMessages({
  library(here)
  library(tidyverse)
  })
...
Using a cutoff as in the package vignette, seem acceptable. We can now move on, define the model and extract the parameters.
...
save(d,design_full,file=here("www/dmDSdata.rda"))
```

```{r precision-setup}
d <- readRDS(file=here("www/dmDSdata.rds"))
```
...
saveRDS(d,file=here("www/dmDSfit.rds"))
```

```{r dtu-setup}
d <- readRDS(file=here("www/dmDSfit.rds"))
```
...
saveRDS(d,file=here("www/dmDStest.rds"))
```

```{r assess-setup}
d <- readRDS(file=here("www/dmDStest.rds"))
```
...
Here we want to ensure that the p-values are uniformely distributed.
...
```{r results, exercise=TRUE, exercise.eval=TRUE, exercise.setup="assess-setup"}
res <- results(d)
res <- res[order(res$pvalue, decreasing = FALSE), ]
top_gene_id <- res$gene_id[3]
```
...
```{r gene, exercise=TRUE, exercise.eval=TRUE, exercise.setup="signif"}
...
```{r ggvenn, exercise=TRUE, exercise.eval=TRUE, exercise.setup="signif"}
...
```{r dropped,exercise=TRUE, exercise.eval=TRUE, exercise.setup="signif"}
(goi <- setdiff(res$gene_id[res$adj_pvalue <= 0.05 & ! is.na(res$adj_pvalue)],padj$geneID))

dev.null <- sapply(goi,function(g){print(plotProportions(d, gene_id = g, group_variable = "group",plot_type = "boxplot1"))})
