...
# to run locally replace www with tutorials/02_exploratory_data_analysis/www
...
# helpers
source(here("www/featureSelection.R"))
...
We have translated the information from the study abstract into a file called [samples_table.tsv](www/samples_table.tsv), which looks like:

```{r sample,echo=FALSE}
read_tsv(here("www/samples_table.tsv"),show_col_types=FALSE)
```
...
```{r metadata2, exercise=TRUE, exercise.eval=TRUE,message=FALSE}
sample_file <- here("www/samples_table.tsv")
samples <- read_tsv(sample_file)
```
...
```{r tximport, exercise=TRUE, exercise.eval=FALSE}
txi <- readRDS(here("www/tximport.rds"))
...
```{r counts, exercise=TRUE, exercise.eval=FALSE}
...
```{r sanity, exercise=TRUE, exercise.eval=FALSE}
...
```{r tibble,  exercise=TRUE, exercise.eval=FALSE}
...
```{r barplot, exercise=TRUE, exercise.eval=FALSE, exercise.lines = 5}
dat <- tibble(x=colnames(counts),y=colSums(counts)) %>% 
  bind_cols(samples)
...
```{r density, exercise=TRUE, exercise.eval=FALSE, exercise.lines=10}
dat <- as.data.frame(log10(counts)) %>% 
  rownames_to_column() %>% 
  pivot_longer(starts_with("SRR"),
               names_to="sample_id",
...
Now that we know the theory, put it in practice using the `DESeq2` package. First, create a `DESeqDataSet` object from your `txi` and samples `object`. It will require that we set a `design`. For this, recall the very beginning of this tutorial and that we agreed on the fact that the variables `treatment` and `time` as being of interest. The design is what tells `DESeq2` how we want to model the expression values.
...
```{r dds, exercise=TRUE, exercise.eval=FALSE}
...
```{r dds-what, exercise=TRUE, exercise.eval=FALSE}
dds
colData(dds)
colnames(dds)
head(rownames(dds))
...
```{r size_factors, exercise=TRUE, exercise.eval=FALSE}
...
```{r norm_factor, exercise=TRUE, exercise.eval=FALSE}
...
```{r vst, exercise=TRUE, exercise.eval=FALSE}
vsd <- varianceStabilizingTransformation(dds, blind=TRUE)
vst <- assay(vsd)
vst <- vst - min(vst)
```
...
```{r htrscdstc, exercise=TRUE, exercise.eval=FALSE}
library(vsn)
...
```{r prcomp, exercise=TRUE, exercise.eval=FALSE}
pc <- prcomp(t(vst))
(percent <- round(summary(pc)$importance[2,]*100))
```
...
```{r scree, exercise=TRUE, exercise.eval=FALSE}
...
```{r twod, exercise=TRUE, exercise.eval=FALSE}
...
```{r twod_rep, exercise=TRUE, exercise.eval=FALSE}
...
```{r featSel, exercise=TRUE, exercise.eval=FALSE}
source(here("www/featureSelection.R"))
conds <- factor(paste(dds$time,dds$treatment))
...
```{r hm, exercise=TRUE, exercise.eval=FALSE,exercise.timelimit=600}
