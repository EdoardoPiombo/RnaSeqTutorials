...
<!-- Edoardo
This question was a quiz originally, but since the answer would change with every group I though of jsut leaving an open question.-->
...
```{r which-comparison, exercise=TRUE}
library(here)
library(readr)
metadata <- read_csv(here('your_metadata_file.csv'))
...
sample_file <- here("your_metadata_file")
metadata <- read_csv(sample_file)
```
...
# using a string
samples <- read_csv(sample_file,col_types=c("cfff"))
samples
# setting a default overall
samples <- read_csv(sample_file,col_types=cols(.default=col_factor()))
samples
...
samples <- read_csv(sample_file,col_types=cols(sample_id=col_character(),
...
```{r txi_import, exercise= TRUE}

tx2gene <- read_delim(here("YOUR_TX2GENE_FILE"),delim="\t",
                                 col_names=c("TXID","GENE"))
filelist <- list.files(here("YOUR_SALMON_FOLDER"), 
                          recursive = TRUE, 
                          pattern = "quant.sf",
                          full.names = TRUE)
...
# Let's also save the samples, in the right order and with a column indicating their salmon quant files, as a new file:
...
<!-- Edoardo
In the next part again the answers will change depending on the group
-->
...
<!-- Edoardo
I commented the following part, as they should have already done that at this point
...
dat <- as.data.frame(log10(counts)) %>% 
  rownames_to_column() %>% 
  pivot_longer(starts_with("FIRST_LETTERS_IN_COMMON_IN_ALL_COLUMNS"),
               names_to="sample_id",
               values_to="x") %>% 
  left_join(samples %>% select(-replicate),by=c("sample_id"))
```
...
<!-- Edoardo
I comment the next part, as for them it will be easier to just now to use the
...
samples$CONDITION_OF_INTEREST <- relevel(samples$CHANGEME, ref = "YOUR_CONTROL_CONDITION")
...
save(dds,file="PATH_TO_PROJECT_FOLDER/dds.rda")
```
...
```{r norm_factor-solution}
 ggplot(as.data.frame(normalizationFactors(dds)) %>% 
          rownames_to_column() %>% pivot_longer(starts_with("FIRST_LETTERS_IN_COMMON_IN_ALL_COLUMNS"),
                                                names_to="sample_id",
...
vst <- assay(vsd)
vst <- vst - min(vst)
save(vst,file="PATH_TO_PROJECT_FOLDER/analysis/DE/vst-aware.rda")
```
...
<!-- Edoardo
I comment the next part, as they will not have these exact files

```{r vst-rds, include=FALSE}
counts <- readRDS(here("inst/tutorials/02_exploratory_data_analysis/www/counts.rds"))
dds <- readRDS(here("inst/tutorials/02_exploratory_data_analysis/www/dds.rds"))
vst <- readRDS(here("inst/tutorials/02_exploratory_data_analysis/www/vst.rds"))
```
-->
...
<!-- Edoardo
We could maybe skip this part, given that they will only have 1 variable?
-->
...
conds <- factor(paste(dds$CONDITION_OF_INTEREST))
sels <- rangeFeatureSelect(counts=vst,conditions=conds, nrep=NUMBER_OF_REPLICATES_PER_CONDITION)
```
