...
tutorial:
  id: "02_exploratory_data_analysis"
  version: 0.1
output:
  learnr::tutorial:
    progressive: true
...
<!-- Here I just removed stuff about the function -->

Now assign a  numeric value to a and b and then add a line to sum them:
...
Explore the metadata file and consider which comparison you could you do with your data.
...
#Please do not change the name of this file, as it will be needed afterwards
write_csv(metadata, here("inst/tutorials/02_exploratory_data_analysis/metadata.csv"))
...
# using a string
samples <- read_csv(metadata_file,col_types=c("ccf"))
samples
...
samples <- read_csv(metadata_file,col_types=cols(SampleID=col_character(),
...
<!-- Edoardo
I need the solution to be run to make the following code successfull,
but if I do eval=TRUE directly in the solution then the students cannot see the solution, therefore I put it here
-->


```{r metadata_setup, include=FALSE, eval=TRUE}

samples <- read_csv(here("inst/tutorials/02_exploratory_data_analysis/metadata.csv"),show_col_types=FALSE)
```
...
```{r txi_import, exercise= TRUE, exercise.eval=TRUE, exercise.setup = "metadata_setup"}
library(tximport)
...
#The next two command lines appear on the same line in the learnr tutorial and I do not know why
...
#And let's also save the txi object:

# Save the txi object
saveRDS(txi, file = here("inst/tutorials/02_exploratory_data_analysis/txi.rds"))
...
quiz(caption="`tximport` object",
     ...
     question("Looking at the value names, what is the difference between 'abundance' and 'counts'?",
              ...
              incorrect="All are true, as the `countsFromAbundance` salmon parameter was set to `no`"
              ),
     ...
     question("What about 'variance'?",
              ...
              incorrect="Reread what it is just above that quiz :-)"
              )
     )
```
...
<!-- Edoardo:
Similarly to before, I need counts-solution to be run for following code to be effective
-->
```{r counts_setup, eval=TRUE, include=FALSE}
txi <- readRDS(here("inst/tutorials/02_exploratory_data_analysis/txi.rds"))
counts <- round(txi$counts)
samples <- read_csv(here("inst/tutorials/02_exploratory_data_analysis/metadata.csv"),show_col_types=FALSE)
```
...
```{r noexp, exercise=TRUE, exercise.eval=FALSE, exercise.setup="counts_setup"}
...
```{r tibble,  exercise=TRUE, exercise.eval=FALSE, exercise.setup="counts_setup"}
...
Now that we have the data in the right shape, we can visualise it. For that we can use the density geom, and as we want to visualise the individual samples, we need to have a corresponding `group` aesthetic.
...
Now try to load the data using DESeq2 commands, and then save it as an rds file

```{r dds, exercise=TRUE, exercise.eval=FALSE, exercise.setup="counts_setup"}
...
saveRDS(dds, file = here("inst/tutorials/02_exploratory_data_analysis/dds.rds"))
...
<!-- Edoardo:
This is another code chunk that will be necessary as setup for other exercises

-->

```{r dds_setup, include= FALSE, eval=TRUE}
...
```{r dds-what, exercise=TRUE, exercise.eval=TRUE, exercise.setup="dds_setup"}
#The following commands appear all on the same line in my learnr, I do not know why
dds
...
```{r size_factors, exercise=TRUE, exercise.eval=FALSE, exercise.setup="dds_setup"}
...
```{r norm_factor, exercise=TRUE, exercise.eval=FALSE, exercise.setup="dds_setup"}
dds <- estimateSizeFactors(dds)
```
...
As for dds, we save the vst object in rds format

```{r vst, exercise=TRUE, exercise.eval=TRUE, exercise.setup="dds_setup"}
# The following commands are on the same line in my learnr, I do not know why
vsd <- varianceStabilizingTransformation(dds, blind=TRUE)
...
saveRDS(vst, file = here("inst/tutorials/02_exploratory_data_analysis/vst.rds"))
```
...
```{r vst_setup, include=FALSE}
counts <- as.matrix(read.csv(here("inst/tutorials/02_exploratory_data_analysis/counts.csv"), row.names = 1))
dds <- readRDS(here("inst/tutorials/02_exploratory_data_analysis/dds.rds"))
dds <- estimateSizeFactors(dds)
vst <- readRDS(here("inst/tutorials/02_exploratory_data_analysis/vst.rds"))

```


```{r htrscdstc, exercise=TRUE, exercise.eval=FALSE, exercise.setup="vst_setup"}
library(vsn)
...
```{r prcomp, exercise=TRUE, exercise.eval=TRUE, exercise.setup="vst_setup"}
pc <- prcomp(t(vst))
(percent <- round(summary(pc)$importance[2,]*100))
```
...
```{r featSel, exercise=TRUE, exercise.eval=TRUE, exercise.setup="vst_setup"}
