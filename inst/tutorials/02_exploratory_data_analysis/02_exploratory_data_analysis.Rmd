...
# to run locally replace www with inst/tutorials/02_exploratory_data_analysis/www
...
counts <- readRDS(here("www/counts.rds"))
```
...
counts <- readRDS(file=here("www/counts.rds"))
...
```{r barplot, exercise=TRUE, exercise.eval=FALSE, exercise.lines = 5, exercise.setup="tibble"}
dat <- tibble(x=colnames(counts),y=colSums(counts)) %>% 
  bind_cols(samples)
...
```{r density, exercise=TRUE, exercise.eval=FALSE, exercise.lines=10, exercise.setup="pivot"}
dat <- as.data.frame(log10(counts)) %>% 
  rownames_to_column() %>% 
  pivot_longer(starts_with("SRR"),
               names_to="sample_id",
...
```{r vst, exercise=TRUE, exercise.eval=FALSE, exercise.setup="dds-rds"}
vsd <- varianceStabilizingTransformation(dds, blind=TRUE)
vst <- assay(vsd)
vst <- vst - min(vst)
```
...
```{r hm, exercise=TRUE, exercise.eval=FALSE,exercise.timelimit=600, exercise.setup="featSel"}
