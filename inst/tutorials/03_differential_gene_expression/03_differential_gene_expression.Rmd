...
Heatmap(t(scale(t(vst[rownames(row_sel),1:ncol(dds)]))),
...
```{r DE-counts, exercise=TRUE, exercise.eval=TRUE, exercise_setup= "heatmap"}
row_sel <-some_results %>%
  as.data.frame() %>%
  filter(!is.na(padj)) %>% 
  filter(padj <= 0.01) %>%
...
```{r export, exercise=TRUE, exercise.eval=FALSE, exercise.setup= "heatmap"}
