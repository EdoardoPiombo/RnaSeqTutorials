...
#helper functions
source("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/UPSCb-common/src/R/featureSelection.R")
source("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/UPSCb-common/src/R/volcanoPlot.R")
source("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/UPSCb-common/src/R/gopher.R")
...
load("PATH_TO_PROJECT_FOLDER/dds.rda")
dds
...
**Note:** The default reference is devised by the alphabetic order of the variable, hence the reason why `control` is the default in this image and the treatment `G02` in our data. However, in our dataset we selected our reference condition yesterday, when we ran:

```{choose_reference, eval=FALSE, echo=TRUE}
samples$CONDITION_OF_INTEREST <- relevel(samples$CHANGEME, ref = "YOUR_CONTROL_CONDITION")
```

<!--Edoardo:
Is this image correct? The treatment/genotype value is still present.
In general, I do not know if it is useful to spend so much time on the interaction of two variables, when they have only one.
-->
...
<!--Edoardo: I did not really get the following paragraphth -->
...
When you have moany contracts, the result() function will, by default, only give the results of the last one. To select a different contrast, we can use the `name=` argument to the `results()` function call. To see the names of all the possible contrasts, you can use the function resultsNames(). Give it a try below.
...
write_tsv(some_results,file=here("analysis/some_results.tsv"))
```
...
```{r cutoffs, exercise=TRUE, exercise.eval=TRUE}
...
row_sel <-some_results %>%  filter(!is.na(padj)) %>% 
  filter(padj <= 0.01) %>%
  filter(abs(log2FoldChange) >= 0.5) 
...
```{r DE-counts, exercise=TRUE, exercise.eval=TRUE}
row_sel <-some_results %>%  filter(!is.na(padj)) %>% 
  filter(padj <= 0.01) %>%
  filter(abs(log2FoldChange) >= 0.5) 
...
write_tsv(as.data.frame(some_results) %>% rownames_to_column("geneID"),
          file=here("PATH_TO_PROJECT_FOLDER/results_DE.tsv"))
```
