...
# to run locally replace www with tutorials/03_differential_gene_expression/www
...
**The p-value is the probability of obtaining a test statistic at least as extreme as the one observed, if the null hypothesis is true** (_i.e. if there is not true signal in the data).
...
Clearly, this does not look like a great fit. Nonetheless, we can model that data using a linear model (`lm()`) using the following `formula`: `brain ~ body`; _i.e._ $brain = f(body)$ and then look at its fit. For that we can look at the $\sum{residuals^{2}}$ as well as its $R^{2}$ value.
...
The simple linear model we used $y=ax+b$ seem to be above most values for low body weight and above for high body weight. We could try a polynomial (_poly()_) model (or a more robust natural spline _ns()_)


```{r poly, exercise=TRUE, exercise.eval=TRUE}
...
quiz(caption="Contrast by default",
     ...
     question("What does that contrast represent?",
              ...
              try_again=TRUE),
     question("How do we extraxt a similar information at `T03`?",
...
Take a minute to reflect on how that is different from `timeT04.treatmentG03` 

(T04 at G03 of expression the to contribution specific the, term interaction an is timeT04.treatmentG03 that is answer the <-- read from right to left.)
...
```{r subset-de, exercise=TRUE, exercise.eval=FALSE, exercise.setup="dds_T04"}
