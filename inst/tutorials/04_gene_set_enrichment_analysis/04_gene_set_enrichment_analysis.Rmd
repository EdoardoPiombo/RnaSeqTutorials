...
```{r test, exercise=TRUE, exercise.eval=TRUE, exercise.timelimit=300, exercise.setup="GOdata"}
...
```{r mtest, exercise=TRUE, exercise.eval=TRUE,exercise.setup="test" }
FDR <- p.adjust(score(resultPC),method="BH")
...
```{r wordcloud, exercise=TRUE, exercise.eval=TRUE, exercise.setup="mtest"}
allResSig <- allResSig %>% mutate(scores = -log10(padj))
library(wordcloud)
