...
saveRDS(geneID2GO,file=here("tutorials/04_gene_set_enrichment_analysis/geneID2GO.rds"))
...
Upregulated <- population %>% filter(padj < 0.01) %>% filter(log2FoldChange > 0.5)
Downregulated <- population %>% filter(padj < 0.01) %>% filter(log2FoldChange < 0.5)
geneID2GO <- readRDS(here("tutorials/04_gene_set_enrichment_analysis/geneID2GO.rds"))
...
```{r GOdata, exercise=TRUE, exercise.setup="geneID2GO_setup", exercise.eval=TRUE}
