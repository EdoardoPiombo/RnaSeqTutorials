...
Let us load the DE data we generated. The file is "PATH_TO_PROJECT_FOLDER/results_DE.tsv".
...
```{r load-solution}
DEres <- read_tsv(file=here("PATH_TO_PROJECT_FOLDER/results_DE.tsv"),show_col_types=FALSE)
```
...
<!-- Edoardo: Should we limit ourselves to over representation analyses? -->
...
```{r genesets, exercise=TRUE, exercise.eval=FALSE}
...
Use the following code to load the gene annotation of the species your group is using. Then, filter the dataset so it includes only the genes present in the population:

```{r annotation, exercise=TRUE, exercise.eval=FALSE}

# Spruce GO 
spruce_genes <- read.csv("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/GO_ids/spruce_go_new_ids.csv", header = TRUE)
geneID2GO <- unstack(spruce_genes[,c(1,2)])

# Pine GO 
pine_genes <- read.csv("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/GO_ids/Pinsy01_go_ids_mod.csv", header = TRUE)
geneID2GO <- unstack(pine_genes[,c(1,2)])

# Aspen GO
geneID2GO <- readMappings("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/GO_ids/Potra02_go_ids.tsv")[-1]
...
# Spruce GO 
spruce_genes <- read_csv("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/GO_ids/spruce_go_new_ids.csv")
geneID2GO <- unstack(spruce_genes[,c(1,2)])

# Pine GO 
pine_genes <- read_csv("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/GO_ids/Pinsy01_go_ids_mod.csv")
geneID2GO <- unstack(pine_genes[,c(1,2)])

# Aspen GO
geneID2GO <- readMappings("/mnt/picea/projects/functional-genomics-course/spring2024/data/week2/GO_ids/Potra02_go_ids.tsv")[-1]

ExpressedGenes <- population %>% select(geneID) %>% pull()
geneID2GO <- geneID2GO %>% filter(gene %in% ExpressedGenes)
...
```{r GOdata, exercise=TRUE}
geneList <- factor(as.integer(names(geneID2GO) %in% Upregulated))
...
<!--Edoardo: the following part is not necessary anymnore, given that now we filter the dataset before the test
...
```{r resScore, exercise=TRUE, exercise.eval=FALSE}
allResSig <- allRes %>% filter(padj < 0.05) %>% 
  mutate(scores = setNames(-log10(weightFisher), GO.ID))
...
```{r wordcloud, exercise=TRUE, exercise.eval=FALSE}
...
<!-- Edoardo: I commented out the hands on as they will not have data from multiple models
...
So how did I know about these methods? Mostly by searching on Bioconductor and reading the package [vignette](https://bioconductor.org/packages/release/bioc/html/topGO.html). The package has two functions that can be used to retrieve a list of existing enrichment algorithms and tests.
...
<!-- Edoardo:
I would stop the tutorial here, rather than explaining other methods besides TopGO. I think it is good to have less material on the last day, so people that are lagging behind have a chance to catch up (in week1 they have and entire day just for that).
People with more time can still try, for example, to do a for loop to run the analysis on Upregulared and Downregulated genes at the same time, or they can check out CC and MF terms.
